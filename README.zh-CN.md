...
1. 运行 NSIS 安装包（`src-tauri/target/release/bundle/nsis/*.exe`）完成安装。
2. 打开应用，点击「设置」。
3. 填写 **Python 路径**（如 `python`）和 **Codex-Usage 脚本路径**（如 `codex_usage.py` 的绝对路径）。
4. 可点击「测试数据源」确认脚本能输出有效 JSON，然后保存并点击「立即刷新」。

**说明：**

- 真实额度数据完全依赖你本机配置的 Codex-Usage 脚本；应用只读取脚本 stdout，不上传数据。
- 若移动或重命名脚本所在文件夹，需要在设置中重新配置脚本路径。
- 应用**不会**保存 `auth.json`、token、cookie 或 API key，仅保存上述路径类配置。
- 运行日志写入前会做脱敏处理，不会在界面展示原始 stdout。
