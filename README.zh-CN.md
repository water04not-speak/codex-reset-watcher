...
Codex Reset Watcher 是一个本地优先的 Codex 使用监控与提醒工具，通过历史记录、趋势分析、到期风险预测、桌面通知和系统托盘，帮助用户避免额度浪费并合理安排使用时间。它不绕过限制，不修改或消耗用户额度，也不上传使用数据。

已登录 Codex 的 Windows 用户**通常无需配置即可自动读取真实额度**，不依赖 Codex-Usage 或 Python。边界：需本机已登录、网络可达、上游 API 形态兼容。

**v0.3.0** 新增本地历史、确定性趋势估算、系统托盘、Windows 通知、数据源健康中心和脱敏 CSV/JSON 导出。
...
这个工具的目标是把「还剩什么、使用速度如何、何时可能耗尽、哪些事件需要处理」放到一个可常驻桌面并可扫视的面板里。
...
- **本地历史与趋势**：记录经过筛选的额度快照，展示近 24 小时 / 7 天用量和可解释的耗尽估算
- **到期与窗口提醒**：支持通知去重、勿扰时间和紧急提醒穿透
- **桌面常驻**：系统托盘、开机自启、窗口置顶、启动最小化和关闭行为均可配置
- **数据源健康**：展示真实源 / fallback / demo、刷新耗时、连续失败和严格脱敏的诊断摘要
...
4. 需要时可点击「立即刷新」。
5. 页脚与设置页会显示应用版本号（例如 `v0.3.0`）。

**v0.3.0 的 Windows 安装包尚未签名**，SmartScreen 可能提示未知发布者。MSI 仍定位为管理员 / 企业部署场景，暂不推荐普通非管理员用户安装。
...
会。v0.3.0 已接入 Tauri Host；开机自启、窗口置顶、启动最小化和关闭到托盘均会真实生效。

## 当前限制

- Windows 安装包尚未签名。
- MSI 在普通非管理员权限下可能失败；请优先使用 NSIS 或便携 exe。
- Windows 通知需要用户授予系统权限。
- 少于两个有效快照或跨度不足 5 分钟时不会提供趋势与耗尽预测。
- 默认点击关闭按钮会最小化到托盘，可在设置中改为直接退出。
